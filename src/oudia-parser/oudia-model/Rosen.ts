...
export class OuDia_Rosen {
  ...
  constructor(value: Class) {
    ...
    this.diagramDgrYZahyouKyoriDefault = value.fields.getValue(
      'DiagramDgrYZahyouKyoriDefault',
    );
    this.enableOperation = value.fields.getValue('enableOperation');
    this.operationCrossKitenJikoku = value.fields.getValue(
    ...
    this.eki = value.classes
      .filter((v) => v.name === 'Eki')
      .map((v) => new OuDia_Eki(v));
    this.ressyasyubetsu = value.classes
      .filter((v) => v.name === 'Ressyasyubetsu')
      .map((v) => new OuDia_Ressyasyubetsu(v));
    this.dia = value.classes
      .filter((v) => v.name === 'Dia')
      .map((v) => new OuDia_Dia(v));
  }
}
