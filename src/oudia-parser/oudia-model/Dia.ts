...
export class OuDia_Dia {
  ...
  constructor(value: Class) {
    ...
    this.kudari = new OuDia_Kudari(
      value.classes.find((v) => v.name === 'Kudari')!,
    );
    this.nobori = new OuDia_Kudari(
      value.classes.find((v) => v.name === 'Nobori')!,
    );
  }
}
...
export class OuDia_Kudari {
  ...
  constructor(value: Class) {
    this.ressya = value.classes
      .filter((v) => v.name === 'Ressya')
      .map((v) => new OuDia_Ressya(v));
  }
}
