...
export const TrainTypeService = {
  append(root: Root, data: TrainType): Root {
  ...
  update(root: Root, index: number, data: TrainType): Root {
    if (index < 0 || root.trainTypes.length <= index) {
      throw new RangeError('存在しないインデックス');
    }
    const trainTypes = [...root.trainTypes];
