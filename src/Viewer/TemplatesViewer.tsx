...
export function TemplatesViewer() {
  ...
  const maxX = 2;
  const maxY = globalState.root.templateTrains.length + 1;
  ...
  const [editData, setEditData] = useState(TemplateTrain.default());

  const startEdit = (_: number, y: number) => {
    if (y === maxY - 1) {
      setEditData(TemplateTrain.default());
      setEditState('new');
    } else {
      setEditData(globalState.root.templateTrains[y]);
      setEditState('edit');
    }
  ...
  const deleteData = (y: number) => {
    ...
    globalState.setRoot((previous) => TemplateTrainService.delete(previous, y));
  };
  const insertData = () => {
    setEditData(TemplateTrain.default());
    setEditState('insert');
    ...
    switch (editState) {
      case 'edit': {
        globalState.setRoot((previous) =>
          TemplateTrainService.update(previous, selectedCellY, editData),
        );
      ...
      case 'insert': {
        globalState.setRoot((previous) =>
          TemplateTrainService.insert(previous, selectedCellY, editData),
        );
      ...
      case 'new': {
        ...
        globalState.setRoot((previous) =>
          TemplateTrainService.append(previous, editData),
        );
  ...
  return (
    <>
      <TableViewer
        ...
        data={globalState.root.templateTrains}
        defaultValue={TemplateTrain.default()}
      ...
      <div className="fixed">
        ...
        >
          ...
          >
            ...
            <select
              onChange={(event) => {
                setEditData({ ...editData, trainTypeId: event.target.value });
              }}
              value={editData.trainTypeId}
            >
              <option value="">選択してください</option>
              {globalState.root.trainTypes.map((v) => (
