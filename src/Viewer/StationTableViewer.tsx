...
export function StationTableViewer() {
  ...
  const deleteData = (y: number) => {
    ...
    globalState.setRoot((previous) => StationService.delete(previous, y));
  };
  const insertData = () => {
    setEditData(Station.default());
    setEditState('insert');
    ...
    switch (editState) {
      case 'edit': {
        globalState.setRoot((previous) =>
          StationService.update(previous, selectedCellY, editData),
        );
      ...
      case 'insert': {
        globalState.setRoot((previous) =>
          StationService.insert(previous, selectedCellY, editData),
        );
      ...
      case 'new': {
        ...
        globalState.setRoot((previous) =>
          StationService.append(previous, editData),
        );
