...
pub use line::{
    ...
    pop_back_line_segment, pop_front_line_segment, push_back_line_segment,
    push_front_line_segment, validate_line,
};
...
pub use template_train::{
    add_template_train, delete_template_train, find_template_train_by_name,
    get_template_segments, pop_back_template_segment, pop_front_template_segment,
    push_back_template_segment, push_front_template_segment, validate_template_train,
};
...
mod tests {
    ...
    fn command_manager_handles_station_add_and_undo() {
        let mut manager = crate::command::CommandManager::new(Box::new(crate::command::EmptyEventEmitter));
        let station = Station::new(StationId::new(WeaverailId::new(1)), "梅田");

        manager.execute(Box::new(crate::command::station::AddStationCommand::new(station.clone())));
